...
function InventoryControl2() {
  ...
  const [selectedEntry, setSelectedEntry] = useState<InventoryType | null>(null);
  ...
  const [itemsCheckedOutByUser, setItemsCheckedOutByUser] = useState<InventoryType[]>([]);

  // For data:
  const [inventoryList, setInventoryList] = useState<InventoryType[]>([]);
  const [tagsToFilter, setTags] = useState<string[]>([]);
  const [searchQuery, setSearchQuery] = useState<string>("");
  const [filteredList, setFilteredList] = useState<InventoryType[]>([]);
  ...
  const handleAddingNewEntryToList = async (entry: InventoryType) => {
    await addDoc(collection(db, "inventoryEntries"), entry);
    setAddFormVisibility(false);
    setIsOpen(false);
  };

  const handleEditingEntryInList = async (entry: InventoryType) => {
    const entryRef = doc(db, "inventoryEntries", entry.id!);
    // Typing for data being updated
    const data: Partial<InventoryType> = {
      name: entry.name,
  ...
  //     const returnEntryData: Partial<InventoryType> = {
  //       isCheckedOut: false,
  //       checkedOutBy: null,
  //       dateCheckedOut: null,
...
export default InventoryControl2;
